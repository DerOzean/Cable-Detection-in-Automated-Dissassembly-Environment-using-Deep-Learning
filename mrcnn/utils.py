...
def unmold_mask(mask, bbox, image_shape):
    ...
    return full_mask
def unmold_mask_fail(mask, bbox, image_shape):
    """Converts a mask generated by the neural network to a format similar
    to its original shape.
    mask: [height, width] of type float. A small, typically 28x28 mask.
    bbox: [y1, x1, y2, x2]. The box to fit the mask in.
    ...
    #for i,cnt1 in enumerate(mask):
     #   x = i    
      #  if i != LENGTH-1:
       #     for j,cnt2 in enumerate(mask[i+1:]):
        #        x = x+1
    ...
    #for i in range(maximum):
     #   pos = np.where(status==i)[0]
      #  if pos.size != 0:
       #     cont = np.vstack(mask[i] for i in pos)
            #hull = cv2.convexHull(cont)
           # unified.append(cont)#hull)
    # Put the mask in the right location.
    #full_mask = np.zeros(image_shape[:2], dtype=np.bool)
    #full_mask[y1:y2, x1:x2] = unified
    #cv2.drawContours(mask,unified,-1,(0,255,0),2)
